...
//  SEGGER_SYSVIEW_Start(); /* Start recording (also initialized from SEGGER_UART_init function) */
  ...
  /* Create desired tasks */

  status = xTaskCreate(led_green_handler, "LED_green_task", 200, NULL, 0, &led_green_handle);
...
static void led_green_handler(void* parameters)
{
	...
	while(1)
	{
		HAL_GPIO_TogglePin(LD2_GPIO_Port, LED_GREEN_PIN);
		HAL_Delay(1000);  /* The delay in miliseconds */
	}
